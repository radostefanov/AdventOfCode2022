...
def get_items_around(x, y):
    ...
    if x > 0 and (current - ord_item(lines[y][x - 1])) <= 1:
        items.append((x - 1, y))
    if x < len(lines[y]) - 1 and (current - ord_item(lines[y][x + 1])) <= 1:
        items.append((x + 1, y))
    if y > 0 and (current - ord_item(lines[y - 1][x])) <= 1:
        items.append((x, y - 1))
    if y < len(lines) - 1 and (current - ord_item(lines[y + 1][x])) <= 1:
        items.append((x, y + 1))
    return items
